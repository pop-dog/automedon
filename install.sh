#!/bin/sh
# Install the agent-orchestrator engine and its Claude skills.
#
#   ./install.sh
#
# - Builds and installs the `orchestrator` binary to ~/.cargo/bin (on PATH via
...
# The orchestrator binary (a build snapshot — re-run to pick up engine changes).
cargo install --path "$script_dir/crates/orchestrator"
...
for skill in agent-orchestrator autocoder; do
    ...
    ln -s "$script_dir/skills/$skill" "$skills_dir/$skill"
    echo "linked $skills_dir/$skill -> $script_dir/skills/$skill"
done

echo "Installed: orchestrator (binary) and skills (agent-orchestrator, autocoder)."
